...
\caption{\textit{The map shows how the four species analyzed are distributed.
...
\caption{\textit{This map includes all species and gives us values of intensity and specificity.
If a color is predominant, it means that that territory is specific to the species corresponding to the color, while the white areas or areas with a low intensity indicate a shared space or not specific to any species.}}
\label{fig}
\end{figure}

\vspace{1cm}
For an accurate understanding of this map we use the \texttt{legend\_set()} function which will generate a legend able to make us understand how the colors are distributed on the map.

\begin{figure}[H]
\centerline{\includegraphics[scale=.7]{legend.png}}
\caption{\textit{The legend explains how the colors are handled in the single \texttt{colorist} map.}}
\label{fig}
\end{figure}
\vspace{1cm}
...
Similarly to the previous model, the same analyzes were made but this time with the bioclimatic variables for the time span from 2021 to 2040.
After developing the model, it was possible to use colorist for the representation.
\vspace{1cm}
\begin{figure}[H]
\centerline{\includegraphics[scale=.9]{map mult fut.png}}
\caption{\textit{This map defines the forecasts of the distributions of the species investigated in this project, in the period from 2021 to 2040.}}
\label{fig}
\end{figure}


In viewing the map, in our opinion, there was no clear understanding of the change between present and future.
To solve this visualization problem, a difference was made between the plot containing the data projected to the future and the plot with the data at the present time.
Therefore, in the first analysis, a comparison was made between the present and future distributions for each species and subsequently a difference was made that allowed us to see more clearly what was the change in the distribution of species in the two time frames.
\vspace{1cm}
\begin{figure}[H]
\centerline{\includegraphics[scale=.4]{minio nowfutu.png}}
\caption{\textit{Representation of the two distributions at present time (left map) and with future forecasts (right map).
The yellow colored areas represent the points where the species Miniopterus s. is most distributed. while the darker areas represent the areas where absent.}}
\label{fig}
\end{figure}

\begin{figure}[H]
\centerline{\includegraphics[scale=.4]{melapresfut.png}}
\caption{\textit{Representation of the two distributions at present time (left map) and with future forecasts (right map).
...
\vspace{1cm}
\begin{figure}[H]
\centerline{\includegraphics[scale=.4]{quercpresfut.png}}
\caption{\textit{Representation of the two distributions at present time (left map) and with future forecasts (right map).
...
\end{figure}
\begin{figure}[H]
\centerline{\includegraphics[scale=.4]{lagoppresfut.png}}
\caption{\textit{Representation of the two distributions at present time (left map) and with future forecasts (right map).
...
\begin{figure}[H]
\centerline{\includegraphics[scale=.9]{miniodiff.png}}
\caption{\textit{This map represents the difference between the values obtained from the models at the future time and the models at the present time. The darker areas suggest a decrease in the population at that point, while the lighter areas suggest an increase.}}
\label{fig}
\end{figure}
\vspace{1cm}
\begin{figure}[H]
\centerline{\includegraphics[scale=.9]{meladiff.png}}
\caption{\textit{This map represents the difference between the values obtained from the models at the future time and the models at the present time. The darker areas suggest a decrease in the population at that point, while the lighter areas suggest an increase.}}
\label{fig}
\end{figure}
\vspace{1cm}
\begin{figure}[H]
\centerline{\includegraphics[scale=.9]{quercdiff.png}}
\caption{\textit{This map represents the difference between the values obtained from the models at the future time and the models at the present time. The darker areas suggest a decrease in the population at that point, while the lighter areas suggest an increase.}}
\label{fig}
\end{figure}
\vspace{1cm}
\begin{figure}[H]
\centerline{\includegraphics[scale=.9]{lagdiff.png}}
\caption{\textit{This map represents the difference between the values obtained from the models at the future time and the models at the present time. The darker areas suggest a decrease in the population at that point, while the lighter areas suggest an increase.}}
\label{fig}
\end{figure}
\vspace{1cm}
...
\newpage
\section{Discussion}
%da caricare nel primo pomeriggio massimo
