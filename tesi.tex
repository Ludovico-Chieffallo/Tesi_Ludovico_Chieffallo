...
The use of species distribution models (SDMs) to map and monitor the distribution of plants and animals has become increasingly important in communication aimed at understanding environmental change, population dynamics and ecological consequences (Miller, J. (2010), Species distribution models. Compass Geography, 4: 490-509. Https://doi.org/10.1111/j.1749-8198.2010.00351.x).
In addition to predicting the distribution of species, the SDMs have become important for many studies that include the study of climate change, the identification of potential protected areas, the determination of places more or less susceptible to invasions, etc. (Miller, J. (2010), Species distribution models. Compass Geography, 4: 490-509. Https://doi.org/10.1111/j.1749-8198.2010.00351.x).
...
Presence-absence modeling methods include: logistic regression (Brito et al., 1999), generalized linear models and generalized additive models (Guisan et al., 2002), random forest (Breiman, 1999), boosted regression trees (Elith et al. al., 2008), multivariate adaptive regression splines (Moisen and Frescino, 2002), and artificial neural networks (Tarroso et al., 2012).
The generalized linear model (GLM) was chosen for this project.
\subsubsection{SDM structure}

The development process of an SDM is mainly composed of 4 phases: preparation of the data necessary for the project, calculation of the models, evaluation of the models and finally application of the models. Doing the data preparation is the part that will take the longest (QUOTE)
...
Species occurrence records are indispensable for calculating a correlative niche model. There are several sources of species presence records; for this project it was decided to use the GBIF-Global Biodiversity Information Facility online database (Yesson et al., 2007)
Once the presences were obtained, the pseudo-absences were created.
Pseudo-absences must be created with caution as their positioning can strongly influence the results of the models (Chefaoui and Lobo, 2008; VanDerWal et al., 2009; Barbet-Massin et al., 2012).
...
The best number of pseudo absences depends on the modeling technique (Barbet-Massin et al., 2012; Kanagaraj et al., 2013).
...
As for the environmental values, for our project the bioclimatic variables taken from WorldClim were chosen (site).Most modeling algorithms are sensitive to high levels of correlation between predictive variables (Dormann et al., 2013.Therefore the correlation of one or more predictor variables can drastically change the quality of the model.High correlations can have two main consequences (Dormann et al., 2013; De Marco and Nobrega, 2018):
...
Variables with very high correlation values (such as -0.7 / -0.8) should be excluded (Dormann et al., 2013).
Another alternative used was that of PCA (principal component analysis) (Vale et al., 2016; Pérez i de Lanuza et al., 2018).
The PCA summarizes the environmental variables in orthogonal factors, therefore completely unrelated, and these factors can be used to model the niche of the species. However, the interpretation of the model will be more difficult. (quote)
Other methodologies have also been tried to understand correlation; Parametric (eg Pearson) or non-parametric (eg Spearman) coefficients were tested (Field et al., 2012).
The variance inflation factor (VIF) measured the correlation of each variable with a combination of all other variables in the model together (Zuur et al., 2010; Dormann et al., 2013). Therefore, VIF measures the multicollinearity or non-independence of the predictive variables. However, the parameters will continue to be fair, consistent and efficient.
The generalized linear model (GLM) was then calculated with the function (MULTIGLM).
The model obtained was then ported to colorist (FUNCTION) and following the workflow, the maps representing the distribution of these species and the level of specificity of each area within the European territory were obtained. 
